...
local function bootOmochat(__)
  local p = Process({
    command = __("_path") .. "omochat/omochat.exe",
    chdir = true,
    hide = false,
  })
  p:spawn()
      ...
      if result then
        if result.status == "done" then
          ...
          elseif filetype == "zip" then
            ...
            local zip  = result[1]
            result  = nil
            local filepath = __("_path") .. "omochat.zip"
            local fh  = io.open(filepath, "wb")
            fh:write(zip)
            fh:close()
            local Conv = require("conv")
            local filepathutf8 = Conv.conv(__("_path"), "UTF-8", "ANSI") .. "omochat.zip"
    ...
    content = function(plugin, ref)
      ...
      if __("_Process") == nil then
        if not(result) then
          ...
          local url = "https://api.github.com/repos/betonetojp/omochat/releases/latest"
          result  = func(url)
        end
      else
        local p = __("_Process")
